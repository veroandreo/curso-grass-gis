...
En esta tercera parte del estudio, utilizaremos R para modelar la distribución de 
*Aedes albopictus* en el norte de Italia. 
Para ello, necesitamos conectarnos a GRASS mediante el paquete `rgrass` para leer
los datos de ocurrencia y los predictores. El paquete *rgrass* está desarrollado
por @rgrass y se puede encontrar en: 
<https://github.com/rsbivand/rgrass/>. Véase la viñeta con más
explicaciones y ejemplos en: <https://rsbivand.github.io/rgrass/>.

# [**rgrass**](https://cran.r-project.org/web/packages/rgrass/index.html)

- `initGRASS()`: inicia una sesión GRASS GIS desde R
- `execGRASS()`: ejecuta comandos del SIG de GRASS 
- `gmeta()`: muestra los metadatos de localización de GRASS
- read_VECT()` y `read_RAST()`: leen mapas vectoriales y raster desde GRASS a objetos *terra* de R.
- write_VECT()` y `write_RAST()`: escriben objetos R *terra* en la base de datos GRASS GIS.

::: {.callout-note}
El paquete `terra` es desarrollado por @terra y eventualmente reemplazará a `raster`.
:::
